...
void cast_uint_bool(T data){
	...
	temp = randomize(temp);
	pd_a3p T[[1]] d (5);
	for(uint i = 0; i < size(declassify(temp));++i){
		if(declassify(temp[i]) % 2 == 0){
			d[i] = 0;
		}
		else{
			d[i] = 1;
	...
	pd_a3p bool[[1]] e (5) = (bool)d;
	
	for(uint i = 0; i < 5; ++i){
		if(declassify(e[i]) == true && declassify(d[i]) == 0){
			result = false;
		}
		if(declassify(e[i]) == false && declassify(d[i]) == 1){
			result = false;
		}
	}
	if(!result){
